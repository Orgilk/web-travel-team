class Header extends HTMLElement {
    ...
    async updateCartCount() {
      try {
          ...
          if (!cartItemCountElement || !cartIcon) {
              console.error('Cart elements not found in the DOM.');
              return;
          }
  
          const count = data.length || 0; // Fallback to 0 if data is not an array
          this.state.cartItemCount = count;
  
          // Update the DOM
          cartItemCountElement.textContent = this.state.cartItemCount;
          console.log('Updated cart count:', this.state.cartItemCount);
  
          // Toggle highlight class
          cartIcon.classList.toggle('cart-highlight', this.state.cartItemCount > 0);
          console.log('Class toggle applied:', this.state.cartItemCount > 0);
      } catch (error) {
