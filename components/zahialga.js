...
import { handleOptionCardClick, updatePeopleCount, processBooking,calculateTotalPrice } from './eventHandler.js';
...
class BookingSection extends HTMLElement {
        ...
        this.shadowRoot.innerHTML = `
            ...
            <section class="package-details">
                <div class="package-columns">
                    <div class="map">
                        <iframe src="${this.optionsData.map}" width="100%" height="450" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
                    </div>
    ...
    attachEventListeners() {
        this.shadowRoot.querySelectorAll('.option-card').forEach(card => {
            card.addEventListener('click', event => handleOptionCardClick(event, this));
        });

        this.shadowRoot.getElementById('adult-count').addEventListener('input', () => {
            updatePeopleCount(this);
            calculateTotalPrice(this);  // Recalculate price when people count changes
        });
        this.shadowRoot.getElementById('child-count').addEventListener('input', () => {
            updatePeopleCount(this);
            calculateTotalPrice(this);  // Recalculate price when people count changes
        });
        this.shadowRoot.getElementById('bookBtn').addEventListener('click', () => processBooking(this));
    }
}
